...
BEGIN
	...
	IF NEW.status='IN-SB' OR NEW.status='WAITING' THEN         
		   INSERT INTO logs.last_job_executed(job_name,created_at,auditupdate) 
		   VALUES ('job-monitor-simpang-bayah',NOW(),NOW()) ON CONFLICT(job_name) DO UPDATE SET 
		   auditupdate=EXCLUDED.auditupdate;
	ELSE
		   INSERT INTO logs.last_job_executed(job_name,created_at,auditupdate) 
		   VALUES ('job-monitor-cp',NOW(),NOW()) ON CONFLICT(job_name) DO UPDATE SET 
		   auditupdate=EXCLUDED.auditupdate;
	END IF;
